...
## 9. 分镜图设计节点拖拽粘连和翻页需点两次

**文件:** `components/Node.tsx`

**问题:** 分镜图设计节点在图片区域点击后节点会跟随鼠标移动且无法取消；翻页按钮需要点击两次才能翻页。

**原因:** 图片容器和翻页按钮都缺少 `onMouseDown stopPropagation`，鼠标事件冒泡到节点容器触发拖拽逻辑，第一次点击被拖拽消费，第二次才正常执行翻页。

**处理:** 图片容器 `<div>` 添加 `onMouseDown={(e) => e.stopPropagation()}`，上一页和下一页按钮各添加 `onMouseDown={(e) => e.stopPropagation()}`。
